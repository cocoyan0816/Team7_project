...
select
    ...
    sum(case when is_returned = 1 then estimated_order_value else 0 end) as estimated_return_value,
    sum(case when is_refunded = 1 then estimated_order_value else 0 end) as estimated_refund_value
